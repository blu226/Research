...
def write_to_file(bus_file, similarity, startTime, filename, rows, sim_len):

    bus_arr = bus_file.split('.')
    busID = bus_arr[0]
    messageLine = str(busID) + "\t\t" + str(startTime) + "\t\t\t" + str(sim_len) + "\t\t" + str(similarity) + "\t\t\t" + str(rows) + "\n"
...
for day in days:
    ...
    f = open(filename, 'w')
    f.write("Bus ID\t\tStart Time\tSim Len\tSimilarity\t\tNum Rows\n")
    f.close()
    ...
    for startTime in range(420,1200,30):
        ...
        for bus_file in buses:
            ...
            for simulation_length in range(30, 210, 30):
                ...
                if time_end - time_begin > amount_of_data_needed:
                    ...
                    for i in range(simulation_length):
                        ...
                        if bus_round1[i][0] != -1 and bus_round2[i][0] != -1:
                            ...
                            dist = funHaversine(float(bus_round1[i][1]), float(bus_round1[i][0]), float(bus_round2[i][1]),
                                                float(bus_round2[i][0]))
                    ...
                    if num_rows_w_data > 0:
                        ...
                        if similarity > similarity_min:
                            write_to_file(bus_file, similarity, startTime, filename, num_rows_w_data, simulation_length)
                            draw_line = True
