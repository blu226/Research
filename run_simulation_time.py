...
def run_simulation_files(mules, T, max_nodes):
    ...
    for ind in range(0, 1):
        ...
        if ind == 0:
            S = [0, 1, 2, 3]
            path_to_folder = "BandsTime" + str(mules) + "/" + str(run) + "/ALL/"
            print("\nALL -----------------------")

        elif ind == 1:
            S = [0]
            path_to_folder = "BandsTime" + str(mules) + "/" + str(run) + "/TV/"
            print("\nTV ----------------------  ")

        elif ind == 3:
            S = [1]
            path_to_folder = "BandsTime" + str(mules) + "/" + str(run) + "/ISM/"
            print("\nISM ------------------------ ")

        elif ind == 2:
            S = [2]
            path_to_folder = "BandsTime" + str(mules) + "/" + str(run) + "/LTE/"
            print("\nLTE ----------------------------")

        elif ind == 4:
            S = [3]
            path_to_folder = "BandsTime" + str(mules) + "/" + str(run) + "/CBRS/"
            print("\nCBRS --------------------------- ")

        # Set correct folder names
        link_exists_folder = "BandsTime" + str(mules) + "/" + str(run) + "/"
...
if generate_files == "Y":
    ...
    set_max_nodes = False
    max_nodes = 15
    for max_mules in range(5, 0, -5):
        for run in range(1, 2):
            print("=============== Folder: Band" + str(max_mules) + " Round: " + str(run))

            S = [0, 1, 2, 3]
            path_to_folder = "BandsTime" + str(max_mules) + "/" + str(run) + "/ALL/"
            link_exists_folder = "BandsTime" + str(max_mules) + "/" + str(run) +"/"
