...
def run_simulation_files(mules, T, max_nodes, run):
    ...
    for ind in band_types:
        ...
        lex_data_directory_day = "Lexington" + str(mules) + "/" + str(run) + "/Day3/"
        validate_data_directory = "Lexington" + str(mules) + "/" + str(run) + "/Day3/"
...
if generate_files == "Y":
    ...
    for max_mules in mule_set:
        for run in range(run_start_time, 2):
            ...
            lex_data_directory_day = "Lexington" + str(max_mules) + "/" + str(run) + "/Day3/"
            ...
            with open("constants.py", "w") as f:
                ...
                f.write("lex_data_directory_day = '" + str(lex_data_directory_day) + "'\n")
                f.write("link_exists_folder = '" + str(link_exists_folder) + "'\n")
                f.write("delivery_file_name = " + '"delivery_day3.txt"' + "\n")
                f.write("metrics_file_name = " + '"metrics_LLC_day3.txt"' + "\n")
                f.write("VMIN = 100" + "\n")
                f.write("VMAX = 300" + "\n")
