...
if generate_files == "Y":
    ...
    for max_mules in mule_set:
        for run in range(1, 2):
            ...
            link_exists_folder_max_nodes = "../Bands" + str(max_nodes) + "/" + str(run) + "/Day1/"
            ...
            with open("constants.py", "w") as f:
                for line in lines:
                    if ("path_to_folder" not in line) and ("S = " not in line) \
                            and ("link_exists_folder" not in line) and ("link_exists_folder_max_nodes" not in line) \
                ...
                f.write("lex_data_directory_day = '" + str(lex_data_directory_day) + "'\n")
                f.write("link_exists_folder = '" + str(link_exists_folder) + "'\n")
                f.write("link_exists_folder_max_nodes = '" + str(link_exists_folder_max_nodes) + "'\n")
