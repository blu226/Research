        ...
        for band in band_folders:
            if os.path.exists(so_far_folder + band):
                ...
                for routing_folder in routing_folders:
                    ...
                    if "ALL" == band and "XChants" == routing_folder and run == 1:
                        metric_file = open(so_far_folder + "/" + band + "/" + routing_folder + "/metrics_LLC_day2_X-CHANTS.txt")

                    elif routing_folder == "Epidemic" and run == 1:
                        metric_file = open(so_far_folder + band + "/" + routing_folder + "/metrics_LLC_day2_Epi.txt")
                    elif "ALL" == band and "XChants" == routing_folder and run == 0:
                        metric_file = open(so_far_folder + "/" + band + "/" + routing_folder + "/metrics_LLC_day1_X-CHANTS.txt")
                    ...
                    print("Metric file loc: ", so_far_folder + "/" + band + "/" + routing_folder)
                    ...
                    for line in lines:
                        ...
                        if "180" in line_arr[0]:
                            if "XChants" == routing_folder:
                                if "ALL" == band and run == 0:
                                    Xchants[t][run] = line_arr[p_id]
                                elif "ALL" == band and run == 1:
...
for i in range(len(Xchants)):
    ...
    Xchants_pk_mean.append(np.mean(Xchants_pk[i]))
    Xchants_pk_SD.append(np.std(Xchants_pk[i]))
...
plt.errorbar(x, Xchants_pk_mean, Xchants_SD, marker='o', linestyle='-', linewidth=2)
...
if p_id == 1 or p_id == 4:
    plt.legend(["X-CHANT","X-CHANT_pk", "S-ER", "CBRS", "ISM", "LTE", "TV"], loc="upper left", fontsize=15, ncol = 2)
elif p_id == 2:
    plt.legend(["X-CHANT", "S-ER", "CBRS", "ISM", "LTE", "TV"], loc="upper center", fontsize=15, ncol = 3)
elif p_id == 3:
    plt.legend(["X-CHANT", "S-ER", "CBRS", "ISM", "LTE", "TV"], loc="upper center", fontsize=15, ncol = 3)
elif p_id ==4:
    plt.legend(["X-CHANT", "S-ER", "CBRS", "ISM", "LTE", "TV"], loc="upper left", fontsize=18, ncol=2)
